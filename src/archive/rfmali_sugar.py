...
# OT solver
from ..hiref.adaptive_HiRef import solve_compressed_hiref
...
class RFMALI(object):
    ...
    def _oversample_geometric(self, X, Y, target_n):
        """
        Oversamples X to match target_n using Geometric Interpolation (Label-Free).
        
        Logic:
        1. Pick a random point from X.
        2. Find its nearest neighbors.
        3. Create a new point randomly on the line between the point and a neighbor.
        4. Repeat until we reach target_n.
        """
        n_current = X.shape[0]
        ...
        if self.verbose:
            print(f"  > Geometric Oversampling: Generating {n_needed} synthetic points...")

        # 1. Fit NN to find local geometry
        from sklearn.neighbors import NearestNeighbors
        nn = NearestNeighbors(n_neighbors=5, n_jobs=self.n_jobs).fit(X)
        
        # 2. Randomly select base points to breed from
        # We allow replacement because we might need to sample the whole dataset multiple times
        base_indices = np.random.choice(n_current, size=n_needed, replace=True)
        base_points = X[base_indices]
        
        # 3. Find neighbors for these base points
        # indices_matrix shape: (n_needed, 5)
        _, indices_matrix = nn.kneighbors(base_points)
        
        # 4. Pick one random neighbor for each base point (excluding itself at col 0)
        # We pick from columns 1-4
        neighbor_cols = np.random.randint(1, 5, size=n_needed)
        neighbor_indices = indices_matrix[np.arange(n_needed), neighbor_cols]
        neighbor_points = X[neighbor_indices]
        
        # 5. Interpolate (Geometric SMOTE)
        # alpha is a random number between 0 and 1 for each point
        alpha = np.random.uniform(0.1, 0.9, size=(n_needed, 1))
        
        # X_new = X_base + alpha * (X_neighbor - X_base)
        X_new = base_points + alpha * (neighbor_points - base_points)
        
        # 6. Handle Labels (If they exist, propagate parent label; else fill -1)
        if Y is not None:
            # We just inherit the label of the base point
            Y_new = Y[base_indices]
            Y_final = np.concatenate([Y, Y_new])
        else:
            Y_final = None

        X_final = np.vstack([X, X_new])
    ...
    def fit(self, x_a, x_b, y_a, y_b):
        # 1. Store original sizes so we can slice later
        self.n_a_orig = x_a.shape[0]
        self.n_b_orig = x_b.shape[0]
        
        # 2. Oversample the smaller domain
        if x_a.shape[0] < x_b.shape[0]:
            # A is smaller -> Oversample A
            x_a, y_a = self._oversample_geometric(x_a, y_a, target_n=x_b.shape[0])
            self.oversampled_domain = 'A'
            
        elif x_b.shape[0] < x_a.shape[0]:
            # B is smaller -> Oversample B
            x_b, y_b = self._oversample_geometric(x_b, y_b, target_n=x_a.shape[0])
            self.oversampled_domain = 'B'
        else:
            self.oversampled_domain = None

        # Update current sizes (Now n_a == n_b)
        ...
        self.T_sparse = solve_compressed_hiref(post_a, post_b, random_state=self.random_state, verbose=self.verbose)
    ...
    def fit_transform(self, x_a, x_b, y_a, y_b):
        ...
        emb_a_full = self.embedding_[:self.n_a]
        emb_b_full = self.embedding_[self.n_a:]
        
        # Truncate to original size
        emb_a_clean = emb_a_full[:self.n_a_orig]
        emb_b_clean = emb_b_full[:self.n_b_orig]

        self.embedding_ = np.vstack([emb_a_clean, emb_b_clean])
