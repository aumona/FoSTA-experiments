...
# SUGAR augmentation
from src.sugar.sugar import sugar_augmentation
...
class RFMALI(object):
    def __init__(self,
        ...
        self.rfgap_params = {
            ...
            'max_normalize': False,  # ensures consistent scaling across datasets with unequal sizes
            'verbose': 0,
            'n_jobs': -1,
        }
    ...
    def fit(self, x_a, x_b, y_a, y_b):
        ...
        # -----------------------------------------------------------
        # 2. SUGAR Augmentation (Transient)
        # -----------------------------------------------------------
        # We need augmented posteriors to feed HiRef, but we don't need
        # full augmented proximities.
        
        if self.n_a < self.n_b:
            n_new = self.n_b - self.n_a
            print(f"Augmenting Domain A with {n_new} samples...")
            x_new = sugar_augmentation(x_a, prox_a, n_new)
            prox_new_cross, prox_new_self = self.rfgap_a.prox_extend(x_new, return_self_prox=True)
            prox_a = sparse.bmat(
                [
                [prox_a, prox_new_cross.T],
                [prox_new_cross, prox_new_self]
                ],format="csr")
            y_a = np.concatenate([y_a, np.full(n_new, -1, dtype=y_a.dtype)])
        elif self.n_b < self.n_a:
            n_new = self.n_a - self.n_b
            print(f"Augmenting Domain B with {n_new} samples...")
            x_new = sugar_augmentation(x_b, prox_b, n_new)
            prox_new_cross, prox_new_self = self.rfgap_b.prox_extend(x_new, return_self_prox=True)
            prox_b = sparse.bmat(
                [
                [prox_b, prox_new_cross.T],
                [prox_new_cross, prox_new_self]
                ],format="csr")
            y_b = np.concatenate([y_b, np.full(n_new, -1, dtype=y_b.dtype)])
        else:
            print("No augmentation needed; domains are of equal size.")
        
        # after augmentation block in fit()
        self.n_a_aug = prox_a.shape[0]
        self.n_b_aug = prox_b.shape[0]
        ...
        rank_schedule = rank_annealing.optimal_rank_schedule(n=post_a.shape[0])
        ...
        self.T_sparse = sparse.csr_matrix((data, (row_idx, col_idx)), shape=(post_a.shape[0], post_b.shape[0]))
    ...
    def fit_transform(self, x_a, x_b, y_a, y_b):
        ...
        # ---- remove virtual points from embedding ----
        n_a_aug = getattr(self, "n_a_aug", self.n_a)
        n_b_aug = getattr(self, "n_b_aug", self.n_b)
        
        keep_a = np.arange(self.n_a, dtype=np.int64)
        keep_b = n_a_aug + np.arange(self.n_b, dtype=np.int64)
        keep = np.concatenate([keep_a, keep_b])
        
        self.embedding_ = self.embedding_[keep]
