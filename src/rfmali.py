...
class RFMALI(object):
    ...
    def fit(self, x_a, x_b, y_a, y_b):
        # Track shapes (supports unequal sizes)
        ...
        # Uneven sizes: compress the larger domain to match the smaller one (centroid-level OT),
        # then lift back to a sparse A<->B coupling using a sparse membership matrix.
        #
        # This keeps things sparse all the way, avoids dense NxN, and gives you a natural way
        # to assign all original points (except if KMeans makes empty clusters; we print diagnostics).

        n_a = self.n_a
        n_b = self.n_b
        rank_schedule = rank_annealing.optimal_rank_schedule(n=min(n_a, n_b))

        if n_a == n_b:
            # --- Balanced case: run HiRef directly on point-level semantics ---
            ...
            if n_a < n_b:
                # ------------------------------------------------------------
                # Compress B -> k=n_a centroids in semantic space (post_b)
                # ------------------------------------------------------------
                k = n_a
                km = MiniBatchKMeans(n_clusters=k,random_state=self.random_state)
                z_b = km.fit_predict(post_b)              # (n_b,)
                post_b_cent = km.cluster_centers_         # (k, C)

                # ------------------------------------------------------------
                # HiRef between A and B-centroids (sizes match)
                # ------------------------------------------------------------
                ...
                T_ac = sparse.coo_matrix(
                    (data, (rows, cols)),
                    shape=(n_a, k)
                ).tocsr()

                # ------------------------------------------------------------
                # Build centroid->points membership M_cb (k x n_b), row-stochastic
                # ------------------------------------------------------------
                counts = np.bincount(z_b, minlength=k).astype(np.float64)
                inv_counts = np.zeros_like(counts)
                inv_counts[counts > 0] = 1.0 / counts[counts > 0]
                M_cb = sparse.coo_matrix(    # Matrix of size (k x n_b), represents cluster memberships (normalized by cluster size -> row-stochastic)
                    (inv_counts[z_b], (z_b, np.arange(n_b, dtype=np.int64))),
                    shape=(k, n_b)
                ).tocsr()

                # Lift back to A->B coupling (n_a x n_b)
                self.T_sparse = (T_ac @ M_cb).tocsr()

                # Diagnostics for empty clusters (can cause dead columns downstream)
                n_empty_clusters = int(np.sum(counts == 0))
                if n_empty_clusters > 0:
                    print(f"[WARN] MiniBatchKMeans produced {n_empty_clusters}/{k} empty clusters in Domain B compression.")

            else:
                # n_b < n_a
                # ------------------------------------------------------------
                # Compress A -> k=n_b centroids in semantic space (post_a)
                # ------------------------------------------------------------
                k = n_b
                km = MiniBatchKMeans(n_clusters=k,random_state=self.random_state)
                
                z_a = km.fit_predict(post_a)              # (n_a,)
                post_a_cent = km.cluster_centers_         # (k, C)

                # ------------------------------------------------------------
                # HiRef between B and A-centroids (sizes match)
                # ------------------------------------------------------------
                rank_schedule = rank_annealing.optimal_rank_schedule(n=n_b)
                (rows, cols, data), _ = HiRef.hiref_lr_fast(
                    post_b, post_a_cent,
                    rank_schedule=rank_schedule,
                    return_coupling=True
                ...
                # This is T_{B->Acent}: (n_b x k)
                T_bc = sparse.coo_matrix(
                    (data, (rows, cols)),
                    shape=(n_b, k)
                ).tocsr()

                # ------------------------------------------------------------
                # Build centroid->points membership M_ca (k x n_a), row-stochastic
                # ------------------------------------------------------------
                counts = np.bincount(z_a, minlength=k).astype(np.float64)
                inv_counts = np.zeros_like(counts)
                inv_counts[counts > 0] = 1.0 / counts[counts > 0]
                M_ca = sparse.coo_matrix(
                    (inv_counts[z_a], (z_a, np.arange(n_a, dtype=np.int64))),
                    shape=(k, n_a)
                ).tocsr()

                # We want A->B coupling. We have B->Acent; transpose to Acent->B, then lift Apoints->Acent.
                # Apoints->Acent is M_ca^T (n_a x k). Acent->B is (T_bc^T) (k x n_b).
                self.T_sparse = (M_ca.T @ T_bc.T).tocsr()

                # Diagnostics for empty clusters
                n_empty_clusters = int(np.sum(counts == 0))
                if n_empty_clusters > 0:
                    print(f"[WARN] MiniBatchKMeans produced {n_empty_clusters}/{k} empty clusters in Domain A compression.")

        # ---------- SPARSE DIAGNOSTICS (RECTANGULAR) ----------
        ...
        print("\nSPARSE COUPLING")
        print("---------------")
        print("Empty rows:", np.sum(row_nnz == 0), "/", n_a_T)
        print("Empty cols:", np.sum(col_nnz == 0), "/", n_b_T)
        print("Rows with >1 nonzero:", np.sum(row_nnz > 1), "/", n_a_T)
        print("Cols with >1 nonzero:", np.sum(col_nnz > 1), "/", n_b_T)
        # NOTE: HiRef sparse out can yield repeated indices; CSR sums duplicates automatically.
        # This print is still useful to see if anything besides {1, 1/size} survives.
        if T.data.size > 0:
            uniq = np.unique(T.data)
            print("Unique nonzero values (first 20):", uniq[:20], ("..." if uniq.size > 20 else ""))
        else:
            print("Unique nonzero values (first 20): []")
        ...
        self.T_sparse /= self.T_sparse.max()

        print("Building joint affinity matrix...")
        # Now T is (na, nb), prox_a is (na, na), prox_b is (nb, nb)
        # Dimensions line up perfectly for dot products
